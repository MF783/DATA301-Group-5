...
```
From the graph above; The speed limit distribution shows the distribution shows the number of crashes at different speed limits. (Note, this is not the speed of the crash, just the speed limit at the location of the crash). The most common speed limit that crashes occur at is 50km/h, followed then by 100km/h. This isn't very surprising. Firstly, the default urban speed limit 50km/h, with a high volume of traffic driving under this speed limit across the country each day. Further, this would also included other types of crashes, such as bikes and busses. This then makes sense that it experiences the most crashes. Majority highways and open roads in New Zealand have a speed limit of 100, and if cars where going in excess of 100km/h then these crashes are also included within the 100km/h range.
...
```
This plot shows the crash severity against advisory speed. There is a slight trend as the advisory speed increases, the crash severity increases too. It's also interesting to note the median advisory speed for non-injury, minor, and serious crashes are all around 55km/h. 
...
```
This plot shows the crash severity against speed limit. just like in the graph above, there is also a slight trend between the speed limit and crash severity.
...
```
From the graph above; The number of crashes on 2 lanes roads is the highest. This makes seance since the majority of crashes happen in 50km/h zones witch are normally in towns and cities. 
...
```
From the graph above; it's clear that cars are the most common vehicle to be involved in an fatal crash compared to all other vehicle types. this is expected sine light passenger vehicles accounted for 76.4% of the vehicles on the road. One interesting observation is the fact that there are more trucks involved in fatal crashes than motorcycles when there are more motorcycles on the road. This could be due to trucks being driven more often and they can cause a lot more damage due to their size/ weaight compared to motorcycles. 
https://www.ehinz.ac.nz/assets/Factsheets/Released_2021/Number-of-Vehicles.pdf
...
```
From the bar graph above; it's clear that fatal car crashes are most likely to happen when the weather is fine. This is somewhat surprising considering it rains 111 days per year on average in New Zealand. However, it could be due to people driving more taking risks when the weather is better.  https://figure.nz/chart/9dXdcHwVoaE17eVO
...
```
From the graph above, we can see that most crashes don't result in any injury. this is probably due tot he fact that most crashes occur when the speed limit is 50km/h or below and this is a relatively low speed. Another reason for the high number of non-injury crashes could be due to the high number of 'fender benders' that are reported.
...
```
This shows a similar story as the graph above, that the non-injury crashes are the highest proportion of crashes that occur, with the proportions of crashes decreasing as the severity gets worse.
